...
public:
	UARTLegacy(const UARTLegacy& source) = delete;
	UARTLegacy(UARTLegacy&& source) = delete;
	UARTLegacy& operator=(const UARTLegacy& source) = delete;
	UARTLegacy& operator=(UARTLegacy&& source) = delete;
	UARTLegacy(Usart* const usart_) 
		: usart(usart_)
	...
	void SendChar(const char ch)
	{
		...
		usart->TDR = ch;
		while(!(usart->ISR & USART_ISR_TC)) {} //just wait
